...
* **El Montón (Heap):** Es una zona de memoria mucho más grande, desordenada y dinámica. La mayoria de lenguajes almacenan aqui los objetos(otros permiten Heap y Stack).

**Ventajas de que se almacene el objeto en el Heap**
- Reservo directamente, el tamaño se decide en ejecución.
- Lo que esta en el Heap, vive mas alla que el método o función donde se ha creado.

**Desventajas de que se almacene el objeto en el Heap**
- Hay que liberar memoria cuando ya no se necesita.
- 
...
En los lenguajes modernos (Java, Python, C#), tú creas objetos constantemente, pero... ¿quién los borra cuando ya no sirven? Si nadie los borra, la memoria RAM se llena y el programa se bloquea (esto se llama *Memory Leak* o fuga de memoria). Puedes borrar esta basura también de forma manual, lo que lo hace mas complicado pero mas eficiente a su vez.
...
**En resumen**
- Dice que el atributo o metodo pertenece a la clase, no a una instancia concreta.
...
* **Su trabajo:** Busca un hueco libre en la memoria RAM del tamaño necesario para guardar tu objeto, reserva ese espacio en memoria e invoca al constructor.
...
Ahora que sabes crear objetos y darles valores iniciales... ¿Qué pasa si intentamos crear un Empleado **sin** darle los apellidos porque aún no los sabemos? Esto nos lleva a la **Sobrecarga de Constructores** (tener varios constructores en la misma clase).
...
Para evitar la **ambigüedad** (Shadowing). A menudo, llamamos a los parámetros de una función igual que a los atributos de la clase. El compilador se lía: ¿te refieres a la variable que acaba de entrar o a la variable guardada en el objeto?.Tis no esta disponible en metodos `static`.
